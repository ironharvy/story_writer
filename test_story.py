...
class MockLM(dspy.LM):
    ...
    def __call__(self, prompt=None, messages=None, **kwargs):
        ...
        if "[[ ## chapter_summary ## ]]" in content or (
            '"chapter_summary"' in content and "2-3 sentence factual summary" in content
        ):
            return [
                '```json\n{"chapter_summary": "Kael infiltrates the cathedral archive to recover the stolen reliquary key before dawn. He confronts Sister Mara in the nave, exposing that the Church staged recent demon attacks to secure more funding. Mara escapes with one fragment, while Kael secures the key and vows to expose the conspiracy in the next district."}\n```'
            ]
        if (
